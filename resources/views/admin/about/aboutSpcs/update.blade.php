...
@endsection
@section('content')
    <form class="form" action="{{ route('admin.about.spc.update') }}" method="POST" id="about_spc_update_from"
                ...
                <div class="d-flex align-items-center flex-wrap mr-2">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        <li class="breadcrumb-item">
                            <a href="{{ route('admin.about.spc') }}" class="text-muted">
                                {{ __('menu.about_spc') }}
                            </a>
                        </li>
                        <li class="breadcrumb-item">
                        ...
                        <div class="card card-custom" id="card_languages">
                            <div class="card-body">
                                ...
                                <ul class="nav nav-tabs" id="myTab2" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link active" id="details_en_tab" data-toggle="tab"
                                            href="#about_spc_details_en">
                                            <span class="nav-icon"><i class="flaticon2-layers-2"></i></span>
                                            <span class="nav-text">{{ __('aboutSpcs.details_en_tab') }}</span>
                                        </a>
                                    </li>
                                    @if (setting()->site_lang_ar == 'on')
                                        <li class="nav-item">
                                            <a class="nav-link" id="details_ar_tab" data-toggle="tab"
                                                href="#about_spc_details_ar" aria-controls="profile">
                                                <span class="nav-icon"><i class="flaticon2-layers-2"></i></span>
                                                <span class="nav-text">{{ __('aboutSpcs.details_ar_tab') }}</span>
                                            </a>
                                        </li>
                                    @endif
                                </ul>
                                ...
                                <div class="tab-content mt-5">
                                    @include('admin.about.aboutSpcs.update_tabs.details_en')
                                    @if (setting()->site_lang_ar == 'on')
                                        @include('admin.about.aboutSpcs.update_tabs.details_ar')
                                    @endif
                                </div>
...
@push('js')
    <script type="text/javascript">
        $('#about_spc_update_from').on('submit', function(e) {
            e.preventDefault();
            ////////////////////////////////////////////////////////////////////
            $('#title_en_error').text('');
            $('#title_ar_error').text('');
                ...
                success: function(data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            customClass: {
                                confirmButton: 'update_about_spc_button'
                            }
                        });
                        $('.update_about_spc_button').click(function() {
                            window.location.href = "{{ route('admin.about.spc') }}";
                        });
                    }
                }, //end success
