...
@endsection
@section('content')
    <form class="form" action="{{ route('admin.books.update') }}" method="POST" id="form_book_update"
                        ...
                        <div class="card card-custom" id="card_languages">
                            <div class="card-body">
                                ...
                                <ul class="nav  nav-tabs" id="myTab2" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link active" id="book_settings_tab" data-toggle="tab"
                                            href="#book_settings">
                                            <span class="nav-icon"><i class="flaticon2-settings"></i></span>
                                            <span class="nav-text">{{ __('books.book_settings_tab') }}</span>
                                        </a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="book_details_en_tab" data-toggle="tab"
                                            href="#book_details_en" aria-controls="profile">
                                            <span class="nav-icon"><i class="flaticon2-layers-1"></i></span>
                                            <span class="nav-text">{{ __('books.book_details_en_tab') }}</span>
                                        </a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="book_details_ar_tab" data-toggle="tab"
                                            href="#book_details_ar" aria-controls="profile">
                                            <span class="nav-icon"><i class="flaticon2-layers-1"></i></span>
                                            <span class="nav-text">{{ __('books.book_details_ar_tab') }}</span>
                                        </a>
                                    </li>
                                </ul>

                                <div class="tab-content mt-5">
                                    @include('admin.books.update_tabs.settings')
                                    @include('admin.books.update_tabs.details_en')
                                    @include('admin.books.update_tabs.details_ar')
                                </div>
                ...
                success: function(data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            customClass: {
                                confirmButton: 'update_book_button'
                            }
                        });
                        $('.update_book_button').click(function() {
