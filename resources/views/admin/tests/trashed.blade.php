                ...
                <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                    ...
                    <li class="breadcrumb-item">
                        <a href="#" class="text-muted">
                            {{ __('menu.trashed_tests') }}
                        </a>
                    </li>

                    <li class="breadcrumb-item">
                        <a href="{!! route('admin.tests.trashed') !!}" class="text-muted">
                            {{ __('menu.show_all') }}
                        </a>
                    </li>
                </ul>
                                                    ...
                                                    <tbody>
                                                        @forelse($trashedTests as $test)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <a class="btn btn-hover-warning btn-icon btn-pill restore_test_btn"
                                                                        data-id="{{ $test->id }}"
                                                                    ...
                                                                    <a href="#"
                                                                        class="btn btn-hover-danger btn-icon btn-pill force_delete_test_btn"
                                                                        data-id="{{ $test->id }}"
                                                        ...
                                                        <tr>
                                                            <td colspan="8">
                                                                <div class="float-right">
                                                                    {!! $trashedTests->appends(request()->all())->links() !!}
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    </tfoot>
...
@push('js')
    <script type="text/javascript">
        ///////////////////////////////////////////////////
        /// delete test
        $(document).on('click', '.force_delete_test_btn', function(e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    // Delete User
                    $.ajax({
                        url: '{!! route('admin.tests.force.delete') !!}',
                        data: {
                            id,
                            id
                        },
        ...
        ////////////////////////////////////////////////////
        // restore test
        $(document).on('click', '.restore_test_btn', function(e) {
            e.preventDefault();
            var id = $(this).data('id');

            $.ajax({
                url: "{{ route('admin.tests.restore') }}",
                data: {
                    id,
                    id
                },
                ...
                success: function(data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            allowOutsideClick: false,
                            customClass: {
                                confirmButton: 'restore_test_button'
                            }
                        });
                        $('.restore_test_button').click(function() {
                            $('#myTable').load(location.href + (' #myTable'));
                        });
                    }
                }, //end success
